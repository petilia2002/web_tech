...
def update_user(payload: UserUpdate, user_id: int = Path(..., ge=1)):
    ...
    if payload.last_name is not None:
        fields.append("last_name = :last_name"); params["last_name"] = payload.last_name
    if payload.first_name is not None:
        fields.append("first_name = :first_name"); params["first_name"] = payload.first_name
    if payload.email is not None:
        fields.append("email = :email"); params["email"] = str(payload.email)
    if payload.login is not None:
        fields.append("login = :login"); params["login"] = payload.login
    if payload.password not in (None, ""):
        fields.append("password_hash = :password_hash"); params["password_hash"] = payload.password
...
def delete_user(user_id: int = Path(..., ge=1)):
    with engine.begin() as conn:
        conn.execute(text("SET search_path TO auth, public"))
        res = conn.execute(text("DELETE FROM auth.users WHERE user_id = :uid"), {"uid": user_id})
        if res.rowcount == 0:
            raise HTTPException(status_code=404, detail="Пользователь не найден")
...
def user_roles(user_id: int = Path(..., ge=1)):
    ...
    return {"items": [
        {"role_id": r["role_id"], "name": r["name"], "is_enabled": bool(r["is_enabled"])}
        for r in rows
    ]}
...
def grant_role(user_id: int = Path(..., ge=1), payload: RoleGrant = ...):
    ...
    try:
        with engine.begin() as conn:
            ...
            if not conn.execute(text("SELECT 1 FROM auth.users WHERE user_id=:uid"), {"uid": user_id}).first():
                raise HTTPException(status_code=404, detail="Пользователь не найден")
            if not conn.execute(text("SELECT 1 FROM auth.roles WHERE role_id=:rid"), {"rid": payload.role_id}).first():
...
def update_role(payload: RoleUpdate, role_id: int = Path(..., ge=1)):
    ...
    if payload.name is not None:
        fields.append("role_name = :name"); params["name"] = payload.name
    if payload.is_enabled is not None:
        fields.append("is_enabled = :is_enabled"); params["is_enabled"] = payload.is_enabled
...
def delete_role(role_id: int = Path(..., ge=1)):
    try:
        with engine.begin() as conn:
            conn.execute(text("SET search_path TO auth, public"))
            res = conn.execute(text("DELETE FROM auth.roles WHERE role_id = :rid"), {"rid": role_id})
            if res.rowcount == 0:
                raise HTTPException(status_code=404, detail="Роль не найдена")
        return {"status": "success", "deleted_role_id": role_id}
    except IntegrityError:
        # если есть связи в auth.user_roles и нет ON DELETE CASCADE
        raise HTTPException(status_code=409, detail="Нельзя удалить роль: есть связанные пользователи")
    except SQLAlchemyError as e:
        raise HTTPException(status_code=500, detail=f"DB error: {e}")
